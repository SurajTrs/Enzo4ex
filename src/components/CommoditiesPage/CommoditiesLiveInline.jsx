...
// Common commodities on TradingView via OANDA
const symbols = [
  { label: "Gold (XAUUSD)", value: "OANDA:XAUUSD" },
  { label: "Silver (XAGUSD)", value: "OANDA:XAGUSD" },
  { label: "Brent (BCOUSD)", value: "OANDA:BCOUSD" },
  { label: "WTI (WTICOUSD)", value: "OANDA:WTICOUSD" },
  { label: "Natural Gas (NATGAS)", value: "OANDA:NATGASUSD" },
  { label: "Platinum (XPTUSD)", value: "OANDA:XPTUSD" },
  { label: "Palladium (XPDUSD)", value: "OANDA:XPDUSD" },
];

export default function CommoditiesLiveInline() {
  const [symbol, setSymbol] = useState(() => localStorage.getItem("commoditiesSelectedSymbol") || symbols[0].value);
  const [theme, setTheme] = useState(() => localStorage.getItem("commoditiesTheme") || "dark");
  const [interval, setInterval] = useState(() => localStorage.getItem("commoditiesInterval") || "60");

  // Sim price
  const [price, setPrice] = useState(2000);
  const [dayOpen, setDayOpen] = useState(2000);
  useEffect(() => {
    const base = 10 + Math.random() * 2500;
  ...
  useEffect(() => { localStorage.setItem("commoditiesTheme", theme); }, [theme]);
  useEffect(() => { localStorage.setItem("commoditiesInterval", interval); }, [interval]);
  useEffect(() => {
    const id = setInterval(() => {
      setPrice((p) => parseFloat(Math.max(0.01, p + (Math.random() - 0.5) * (symbol.includes('USD') ? 2 : 0.5)).toFixed(2)));
    }, 1000);
  ...
  const [watchlist, setWatchlist] = useState(() => {
    try {
      const s = localStorage.getItem("commoditiesWatchlist");
  ...
  return (
    <section id="commodities-live" className="py-16">
        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-50 border border-purple-100 text-purple-700 mb-2">
              ...
              <span className="text-sm font-medium">Live commodities</span>
            </div>
            <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900">Live chart & price</h2>
            <p className="text-gray-600">Select a commodity or enter a TradingView symbol.</p>
          </div>
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <Link to="/commodities/live" className="px-5 py-2 rounded-xl bg-gradient-to-r from-purple-600 to-fuchsia-600 text-white font-semibold shadow-lg shadow-fuchsia-500/20 hover:opacity-95">
              Start trading
            </Link>
          </div>
        </div>
