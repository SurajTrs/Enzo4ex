...
export default function IndicesLive() {
  ...
  const [qty, setQty] = useState(1); // index contracts
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-10">
        <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900">Live Indices Prices & Chart</h1>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            {/* Watchlist & quick indices */}
            <div className="rounded-3xl border border-purple-100 bg-white shadow-sm p-5">
              <div className="font-semibold mb-3">Quick indices</div>
              <div className="flex flex-wrap gap-2 mb-3">
                {symbols.map((s) => (
                  <button key={s.value} onClick={() => setSymbol(s.value)} className={`px-3 py-1.5 rounded-lg text-sm border ${symbol === s.value ? 'bg-purple-600 text-white border-purple-600' : 'border-purple-200 text-purple-700 hover:bg-purple-50'}`}>{s.label}</button>
                ))}
              </div>
              <div className="grid grid-cols-3 gap-3 text-sm">
                <div className="p-3 rounded-xl bg-purple-50 text-purple-700 text-center">Bid<br/><span className="font-semibold">{bid}</span></div>
                <div className="p-3 rounded-xl bg-fuchsia-50 text-fuchsia-700 text-center">Ask<br/><span className="font-semibold">{ask}</span></div>
                <div className="p-3 rounded-xl border border-purple-200 text-gray-800 text-center">Spread<br/><span className="font-semibold">{spread}</span></div>
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-purple-100 bg-white shadow-sm p-5">
              ...
              <div className="mb-3">
                <label className="text-sm text-gray-600">Quantity (contracts)</label>
