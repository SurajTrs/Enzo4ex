...
const symbols = [
  { label: "Gold vs USD (XAUUSD)", value: "OANDA:XAUUSD" },
  { label: "Gold vs EUR (XAUEUR)", value: "OANDA:XAUEUR" },
  { label: "Gold vs JPY (XAUJPY)", value: "OANDA:XAUJPY" },
  { label: "Gold vs AUD (XAUAUD)", value: "OANDA:XAUAUD" },
  { label: "Gold vs CHF (XAUCHF)", value: "OANDA:XAUCHF" },
  { label: "Gold vs GBP (XAUGBP)", value: "OANDA:XAUGBP" },
  { label: "Gold vs CAD (XAUCAD)", value: "OANDA:XAUCAD" },
];

export default function GoldLive() {
  const [symbol, setSymbol] = useState(() => {
    const urlSym = new URLSearchParams(window.location.search).get('symbol');
    return urlSym || localStorage.getItem("goldSelectedSymbol") || symbols[0].value;
  });
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          {symbols.slice(0, 7).map((s) => (
            ...
            >
              {s.label.replace('Gold vs ','').replace(' (',' ').replace(')','')}
            </button>
          ))}
        </div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900">Live Gold Prices & Chart</h1>
            <p className="text-gray-600">Powered by TradingView
              <a href="/classic-leverage" className="ml-3 inline-block text-sm text-purple-700 underline underline-offset-2 decoration-purple-300 hover:text-purple-800">View contract specs</a>
            </p>
          </div>
